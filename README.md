# Lab Notes: Debugging Signal Outputs and Header Intricacies on Nucleo-L552ZE-Q
...
To bypass HAL abstraction and rule out permission blocks, I wrote direct register writes to the GPIO Mode Register (`GPIOx_MODER`) at runtime via code:
* Forcing pin mode bits directly (e.g., transitioning from `0x3` [Analog/Reset] to `0x1` [General Purpose Output]).
* Verified register bit changes in the IDE Special Function Registers (SFR) view.

> **Conclusion:** For pins where `MODER` updated correctly to output mode (`0x1`) in memory but still failed to drive a physical signal at the header socket, the root cause was definitively isolated to **open solder bridges (PCB airgaps)** or **hardwired peripheral multiplexing conflicts**.
